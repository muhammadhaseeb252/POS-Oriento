...
function get_customer_data()
{
    ...
    $row = $query->fetchAll(PDO::FETCH_ASSOC);
    foreach ($row as $row) {
        $data .= '<option value="' . $row["customer_id"] . '">' . $row["name"] . '</option>';
    }
    return $data;
}
    ...
    $(document).ready(function () {
        ...
        $(document).on("keyup", "#qty", function () {
            ...
            if (qty > stk) {
                ...
                tr.find("#tot").val(pur_price * 1);
                tr.find("#total").val(pur_price * 1);
            ...
            if ($("#pur_table > tbody > tr").length > 0) {
                ...
                $.ajax({
                    ...
                    success: function (invoice) {
                        console.log(invoice);
                        if ($.trim(invoice) === 'sale_added') {
                            Swal.fire({
                                title: "Success!",
                                text: "Sale Invoice saved Successfully!",
                                type: "success",
                                customClass: "sweet-alert",
                            }).then(function () {
                        ...
                        else if ($.trim(invoice) === 'sale_not_added') {
                            Swal.fire({
                                title: "Error!",
